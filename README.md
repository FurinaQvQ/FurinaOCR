...
- **内存**：至少 4GB RAM
- **存储**：至少 500MB 可用空间
- **运行库**：.NET Framework 4.7.2 或更高版本（Windows通常预装）
...
### SVTR模型架构
FurinaOCR使用**SVTR（Spatial Vision Transformer）**识别模型：

- **🏗️ 架构**：MobileNetV3_Small + Transformer
- **🎯 精度**：专门针对原神字体训练
- **⚡ 性能**：相比PaddleOCR V4提升**6倍**速度
- **📦 体积**：优化模型，轻量高效
...
- **Git**: 最新版本
- **EditorConfig**: 支持 EditorConfig 的编辑器
...
#### 方法三：从现有程序提取
如果你有现成的 `FurinaOCR.exe`，模型已经嵌入在程序中，但需要专门的工具来提取。
