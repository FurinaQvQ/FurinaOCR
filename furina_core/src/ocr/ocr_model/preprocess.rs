use image::{imageops, GenericImageView, ImageBuffer, Luma, RgbImage};

/// convert rgb image to f32 gray image
pub fn to_gray(raw: &RgbImage) -> ImageBuffer<Luma<f32>, Vec<f32>> {
    let mut new_gray: ImageBuffer<Luma<f32>, Vec<f32>> =
        ImageBuffer::new(raw.width(), raw.height());
...
pub fn pre_process(
    im: ImageBuffer<Luma<f32>, Vec<f32>>,
) -> (ImageBuffer<Luma<f32>, Vec<f32>>, bool) {
    let mut im = im;
